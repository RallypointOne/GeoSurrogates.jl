...
```{julia}
#| label: fig-idw
#| fig-cap: "IDW: Original elevation (left) vs IDW interpolation (center) and error (right)"
...
```{julia}
#| label: fig-rbf
#| fig-cap: "RBF: Original elevation (left) vs RBF interpolation (center) and error (right)"
...
```{julia}
#| label: fig-tps
#| fig-cap: "TPS: Original elevation (left) vs exact TPS (center) and smoothed TPS (right)"
...
A boosting-style composite that fits each model to the residuals of the previous models.
This example combines a `LinReg` (captures the broad trend) with an `ImplicitTerrain.MLP` (captures the fine details the regression misses).
...
fit!(additive, elev_norm; steps=1000)
...
```{julia}
#| label: fig-additive
#| fig-cap: "AdditiveModel: LinReg captures the trend (left), MLP fits the residual (center), combined prediction (right)"
...
| `IDW` | Raster | Scalar | Inverse distance weighting |
| `RBF` | Raster | Scalar | Kernel-based interpolation |
| `TPS` | Raster | Scalar | Thin plate spline interpolation |
...
| `AdditiveModel` | Raster | Scalar | Boosting / residual fitting |
